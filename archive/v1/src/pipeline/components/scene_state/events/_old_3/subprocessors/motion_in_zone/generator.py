from typing import (
    List, Set, Dict, Iterable, NamedTuple, override, Type
)
from datetime import datetime
from warnings import deprecated

from archive.v1.src.pipeline.components.scene_state.events._old_3.subprocessors.motion_in_zone.intermediary_containers import IntermediaryZoneEventsContainer, \
    InputEventsForAliveVehicle, InputEventsForDeadVehicle, InputForAliveVehicleWithVehicleID, \
    InputForDeadVehicleWithVehicleID, InputForVehicles, IntermediaryOutputEventsContainer
from archive.v1.src.pipeline.components.scene_state.events._old_3.subprocessors.motion_in_zone.single_vehicle_status_updater import VehicleStatusUpdater
...
from archive.v1.src.pipeline.components.scene_state.events._old_3.subprocessors.motion_global.events import (
    MotionStatusEventsContainer, MotionStatusUpdate
)
...
class InZoneMotionStatusInput(NamedTuple):
...
class InZoneMotionStatusEventGenerator(
    BaseFinalEventGenerator[InZoneMotionStatusInput, InZoneMotionEventsContainer]
):
    ...
    @override
    @classmethod
    def _get_container_class(cls) -> Type[InZoneMotionEventsContainer]:
        return InZoneMotionEventsContainer
    ...
    def _update_and_get_events_for_alive_vehicles(
        ...
        containers: List[IntermediaryOutputEventsContainer] = []
        for input_for_vehicle in input_events_by_vehicle:  # type: InputForAliveVehicleWithVehicleID
            ...
            vehicle_output: IntermediaryOutputEventsContainer = updater.update_and_get_events(
                input_for_vehicle.events, system_state=system_state
            )
            containers.append(vehicle_output)
        return IntermediaryOutputEventsContainer.concatenate(containers)

    @staticmethod
    def _check_for_dead_vehicle_end_output_events_only(output_for_vehicle: IntermediaryOutputEventsContainer) -> None:
    ...
    def _check_same_zone_ids_in_input_and_output_for_dead_vehicle(
            *, input_for_vehicle: InputEventsForDeadVehicle,
            output_for_vehicle: IntermediaryOutputEventsContainer
    ) -> None:
        # Runtime check:
        #   (ABANDONED)
    ...
    def _update_and_get_events_for_dead_vehicles(
        ...
        for input_for_vehicle in input_events_by_vehicle:  # type: InputForDeadVehicleWithVehicleID
            ...
            # select the updater for this vehicle
            updater: VehicleStatusUpdater = self._vehicle_updaters[vehicle_id]
            # the vehicle is now dead -- end the processing for this updater and get end events
            vehicle_output: IntermediaryOutputEventsContainer = (
    ...
    ) -> InZoneMotionEventsContainer:
        output_for_alive_vehicles: IntermediaryOutputEventsContainer = self._update_and_get_events_for_alive_vehicles(
            input_events_by_vehicle.alive, system_state=system_state
        )
        output_for_dead_vehicles: IntermediaryOutputEventsContainer = self._update_and_get_events_for_dead_vehicles(
            input_events_by_vehicle.dead, cur_frame_ts=system_state.cur_frame_ts
        )
        output_for_all_vehicles: IntermediaryOutputEventsContainer = IntermediaryOutputEventsContainer.concatenate(
    ...
    def _check_input_and_arrange_by_vehicle(
            *, global_motion_events: MotionStatusEventsContainer, zone_events: ZoneTransitEventsContainer
    ) -> InputForVehicles:
        """
        ...
        for motion_update in global_motion_events.status_updates: # type: MotionStatusUpdate
            ...
            mappings_for_alive[motion_update_vehicle_id] = InputEventsForAliveVehicle(
                current_confirmed_motion_status_update_event=motion_update,
    ...
    def _update_and_get_events(
            self, input_obj: InZoneMotionStatusInput, *, system_state: ProcessingSystemState
    ) -> InZoneMotionEventsContainer:
        self._add_updaters_for_new_vehicles(input_obj.lifetime_events.start)
        ...
        input_events_by_vehicle: InputForVehicles = self._check_input_and_arrange_by_vehicle(
            global_motion_events=input_obj.global_motion_events,
            zone_events=input_obj.zone_transit_events
        )
        # produce events and update the updaters
        container: InZoneMotionEventsContainer = self._run_updaters_and_get_events(input_events_by_vehicle,
    ...
    def _get_events_for_end_of_processing(self,
                                          *, event_ts: datetime,
                                          truncate_events: bool) -> InZoneMotionEventsContainer:
        output_events: IntermediaryOutputEventsContainer = IntermediaryOutputEventsContainer()
        containers: List[IntermediaryOutputEventsContainer] = []
        for updater in self._vehicle_updaters.values(): # type: VehicleStatusUpdater
            ...
            containers.append(vehicle_output)
        out_container: InZoneMotionEventsContainer = output_events.to_final_container()
        return out_container

    @override
    def _clear_own_state(self) -> None:
        vehicle_ids: List[str] = list(self._vehicle_updaters.keys())
        for vehicle_id in vehicle_ids: # type: str
            self._vehicle_updaters.pop(vehicle_id)