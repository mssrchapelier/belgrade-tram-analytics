...
from archive.v1.src.pipeline.components.scene_state.live_state_updater._old_3.updaters.speed_updater import (
...
from tram_analytics.v1.models.components.scene_state.live_state.vehicles import AgnosticZoneInfoForVehicle, \
    CurrentVehicleStationaryStats, PreviousVehicleStationaryStats, VehicleStationaryStats
...
class VehicleStationaryTracker:
    ...
    def __init__(self) -> None:
        self._prev_times: PreviousStationaryTimes | None = None
    ...
    def start_stationary(self, cur_ts: float) -> None:
        if self._cur_status is MotionStatus.STATIONARY:
            raise RuntimeError("Received stationary start event but this vehicle's "
                               "lifetime-bound stationary status is already STATIONARY")
        if self._cur_stationary_start_ts is not None:
            raise RuntimeError("Received stationary start event but this vehicle's "
                               "_cur_stationary_start_ts is defined")
        self._cur_status = MotionStatus.STATIONARY
        self._cur_stationary_start_ts = cur_ts

    def end_stationary(self, cur_ts: float) -> None:
        ...
        self._prev_times = PreviousStationaryTimes(
    ...
    def is_stationary(self) -> bool | None:
        # used for exporting to CurrentVehicleStationaryStats
        match self._cur_status:
            ...
            case MotionStatus.STATIONARY:
                return True
            case _:
                raise RuntimeError(f"Got unsupported lifetime-bound stationary status: {self._cur_status}")

    def _export_prev_state(self) -> PreviousVehicleStationaryStats:
        start_ts: datetime | None = (
            posix_to_utc_datetime(self._prev_times.start)
            if self._prev_times is not None
            else None
        )
        end_ts: datetime | None = (
            posix_to_utc_datetime(self._prev_times.end)
            if self._prev_times is not None
            else None
        )
    ...
    def export_state(self) -> VehicleStationaryStats:
        ...
        return VehicleStationaryStats(current=current, previous=previous)
...
class VehicleState:
    ...
    def __init__(self,
        ...
        # stores data for this vehicle's stationary periods (in global terms, i. e. not wrt any specific zone):
        # (1) the start timestamp of the current period (if defined)
        # (2) the start and end timestamps of the previous period (if defined)
        self._stationary_tracker: VehicleStationaryTracker = VehicleStationaryTracker()
    ...
    def start_stationary(self, cur_ts: float) -> None:
        self._stationary_tracker.start_stationary(cur_ts)

    def end_stationary(self, cur_ts: float) -> None:
        self._stationary_tracker.end_stationary(cur_ts)
    ...
    def export_state(self) -> ExportFromVehicleState:
        ...
        stationary: VehicleStationaryStats = self._stationary_tracker.export_state()
