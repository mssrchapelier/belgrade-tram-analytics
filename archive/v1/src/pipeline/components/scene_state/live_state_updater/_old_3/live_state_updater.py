...
from archive.v1.src.pipeline.components.scene_state.events._old_3.subprocessors.motion_global.events import (
    # StationaryStartEvent_Old, StationaryEndEvent_Old, StationaryEventsContainer_Old,
    MotionStatusUpdate
)
...
from archive.v1.src.pipeline.components.scene_state.events._old_3.pipeline.events import EventsContainer
...
from archive.v1.src.pipeline.components.scene_state.live_state_updater._old_3.updaters.vehicle_updater import (
    VehicleState, ExportFromVehicleState, ExportFromVehicleZonesInfoItem
)
from archive.v1.src.pipeline.components.scene_state.live_state_updater._old_3.updaters.zone_updater import SingleEventTypePeriodUpdaters, ZoneState
...
class LiveStateUpdater:
    ...
    def _update_vehicle_motion_status(
            self, *, vehicle_id: str, momentary: MotionStatus, confirmed: MotionStatus
    ) -> None:
        self._check_vehicle_initialised(vehicle_id)
        vehicle_state: VehicleState = self._get_vehicle_state(vehicle_id)
        vehicle_state.update_motion_status(momentary=momentary, confirmed=confirmed)
    ...
    # --- (3) global motion status CHANGES ---
    ...
    def update_and_export_state(self, *, events: EventsContainer, settings: ServerSettings) -> LiveAnalyticsState:
        ...
        # (5) zone occupancy (entrances / exits)
        #     - updates:
