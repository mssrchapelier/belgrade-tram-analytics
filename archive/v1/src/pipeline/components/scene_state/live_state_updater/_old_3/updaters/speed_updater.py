...
from tram_analytics.v1.models.components.scene_state.live_state.speeds import SpeedStats, SpeedStatsWithCurrent, LifetimeSpeeds, \
    InZoneSpeeds
...
class MaxSpeedUpdater(BaseSpeedMetricUpdater):
    ...
    def _update_with_matched(self, current: float | None) -> None:
        ...
        self._max_speed = max(max_over) if len(max_over) > 0 else None
...
class SpeedMetricsUpdaterForSpeedType:
    ...
    def update(self, speed: float | None, is_matched: bool) -> SpeedMetricsForSpeedType:
...
class SpeedMetricsTracker:
    ...
    def update(self,
               *, speed_raw: float | None,
               ...
               is_matched: bool) -> SpeedMetrics:
        metrics_for_raw: SpeedMetricsForSpeedType = self._updater_raw.update(speed_raw, is_matched)
        metrics_for_smoothed: SpeedMetricsForSpeedType = self._updater_smoothed.update(speed_smoothed, is_matched)
        metrics: SpeedMetrics = SpeedMetrics(
