...
from archive.v1.src.pipeline.components.scene_state.events._old_3.subprocessors.motion_in_zone.events import (
    InZoneStationaryStartEvent, InZoneStationaryEndEvent, InZoneMotionEventsContainer
)
from archive.v1.src.pipeline.components.scene_state.events._old_3.pipeline.events import FrameMetadataForEvents, EventsContainer
...
from tram_analytics.v1.pipeline.components.scene_state.live_state_updater.updaters.zone_updater import SingleEventTypePeriodUpdaters, ZoneState
...
class LiveStateUpdater:
    ...
    def _update_from_motion_status_updates(self, events: List[MotionStatusUpdate]) -> None:
        for e in events: # type: MotionStatusUpdate
            self._update_vehicle_motion_status(vehicle_id=e.vehicle_id,
                                               event_ts=self._internal_state.cur_frame_ts,
    ...
    def _extract_inzone_stationary_and_arrange_by_zone_id(
            events: InZoneMotionEventsContainer
    ) -> List[ZoneStateUpdate]:
        # zone_id -> (vehicle_ids_to_add, vehicle_ids_to_remove)
    ...
    def _update_from_inzone_motion_change_events(self, events: InZoneMotionEventsContainer) -> None:
        # NOTE: Only STATIONARY start/end are tracked in this live state updater
    ...
    def update_and_export_state(
            self, *, events: EventsContainer, settings: ServerSettings
    ) -> LiveAnalyticsState:
        ...
        self._update_from_motion_status_updates(events.pipeline_steps.global_motion.status_updates)
        ...
        self._update_from_global_motion_change_events(events.pipeline_steps.global_motion.changes_in_confirmed_status)
        ...
        self._update_from_zone_transit_events(events.pipeline_steps.canonical.zone_transit)
        ...
        self._update_from_inzone_motion_change_events(events.pipeline_steps.motion_in_zone)
