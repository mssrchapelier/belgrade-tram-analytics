...
class BasePipelineStageConfig(BaseModel):
    """
...
class BasePipelineStage[
    ...
    **Implementation**
    ...
    async def _input_consumer(self) -> AsyncIterator[InputT]:
        """
        ...
        while True:
            ...
            # (2) the feeders have been signalled to shut down
            self._logger.debug("Input consumer waiting for control events before attempting consuming ...")
            is_consuming, is_shut_down = await self._listen_on_control_flags_before_attempting_consuming()
            if is_shut_down:
                # is shut down; return immediately
                self._logger.debug("Input consumer got a shutdown signal and is exiting")
                break
            ...
            self._logger.debug("Input consumer waiting for control events while attempting consuming ...")
            done, pending = await asyncio.wait(
                tasks, return_when=asyncio.FIRST_COMPLETED
            )  # type: Set[Task[Any]], Set[Task[Any]]
            ...
            if control_events_task in done:
                ...
                self._logger.debug("Input consumer was signalled to not consume")
                continue
            ...
            self._logger.debug(f"Input consumer got item {item}, passing to executor")
            yield item
