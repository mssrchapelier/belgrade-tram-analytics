...
from tram_analytics.v1.models.components.scene_state.events.scene_events import FrameMetadataForEvents, \
    EventsContainer
from tram_analytics.v1.models.components.scene_state.events.subprocessors.lifetime import VehicleLifetimeStartEvent, \
    VehicleLifetimeEndEvent
from tram_analytics.v1.models.components.scene_state.events.subprocessors.motion_status.motion_global.periods.confirmed import (
    ...
    StationaryStartEvent, StationaryEndEvent
)
...
from tram_analytics.v1.models.components.scene_state.live_state.vehicles import AgnosticZoneInfoForVehicle, \
    ...
    VehiclesContainer
from tram_analytics.v1.models.components.scene_state.live_state.zones import PeriodsForZone, AgnosticZoneMetadata, \
    TrackMetadata, \
...
# - server settings
# from src.v1.api_server.models.scene_state.scene_state_settings import ServerSettings
from tram_analytics.v1.pipeline.components.scene_state.config.scene_events_config import (
    SceneEventsConfig
)
# - updater config and child modules
from tram_analytics.v1.pipeline.components.scene_state.live_state_updater.config.zones_config import \
    SingleZoneConfig, ZonesConfig
from tram_analytics.v1.pipeline.components.scene_state.live_state_updater.updaters.vehicle_updater import (
    VehicleState, ExportFromVehicleState, ExportFromVehicleZonesInfoItem
)
from tram_analytics.v1.pipeline.components.scene_state.live_state_updater.updaters.zone_updater import \
    SingleEventTypePeriodUpdaters, ZoneState
...
class UpdaterInternalState:
    ...
    def __init__(self, camera_id: str) -> None:
        ...
        self._cur_server_settings: SceneEventsConfig | None = None
    ...
    @property
    def cur_server_settings(self) -> SceneEventsConfig | None:
        return self._cur_server_settings

    def update_for_frame(self, *, events: EventsContainer, settings: SceneEventsConfig) -> None:
        """
        Updates the internal state with data for the new frame (with input validation)
        and the current server settings.
        """
        frame_metadata: FrameMetadataForEvents = events.metadata
        if frame_metadata.frame_id is None:
            raise ValueError("Frame ID cannot be null for the live state updater")
        if frame_metadata.frame_ts is None:
            raise ValueError("Frame timestamp cannot be null for the live state updater")

        # camera id must stay the same
        stored_cam_id: str | None = self._camera_id
        cur_cam_id: str = frame_metadata.camera_id
        if stored_cam_id != cur_cam_id:
            raise ValueError("Passed an unexpected camera ID "
                             f"(expected {stored_cam_id}, got {cur_cam_id})")

        # the frame timestamp must not have decreased
        prev_ts: float | None = self._cur_frame_ts
        cur_ts: float = datetime_to_utc_posix(frame_metadata.frame_ts)
        if prev_ts is not None and cur_ts < prev_ts:
            raise ValueError("The current frame timestamp must not be earlier than the previous one "
                             f"(previous {prev_ts}, received current: {cur_ts})")
        cur_frame_id: str = frame_metadata.frame_id
...
class MotionStatusInfoForVehicle(NamedTuple):
    vehicle_id: str
    momentary: MotionStatus
    confirmed: MotionStatus

def _motion_status_updates_container_arrange_by_vehicle(
        container: MotionStatusUpdatesContainer
) -> List[MotionStatusInfoForVehicle]:
    """
    Extract the current momentary and confirmed global motion status from events,
    arrange by vehicle ID and return as a list of items `(vehicle_id, momentary, confirmed)`.
    """
    # extract momentary and confirmed status and map to vehicle id
    vehicle_id_to_momentary_status: Dict[str, MotionStatus] = {
        update.vehicle_id: update.motion_status
        for update in container.momentary.updates
    }
    vehicle_id_to_confirmed_status: Dict[str, MotionStatus] = {
        update.vehicle_id: update.motion_status
        for update in container.confirmed.updates
    }

    # --- validity checks ---
    # (1) check that the vehicle ids are the same
    vehicle_ids_with_momentary: Set[str] = set(vehicle_id_to_momentary_status.keys())
    vehicle_ids_with_confirmed: Set[str] = set(vehicle_id_to_confirmed_status.keys())
    if not vehicle_ids_with_momentary == vehicle_ids_with_confirmed:
        raise ValueError("Invalid global motion status updates input: "
                         "the sets of vehicle IDs for momentary and confirmed status updates are not the same")
    # (2) check that there are no duplicates
    if len(vehicle_ids_with_momentary) != len(container.momentary.updates):
        raise ValueError("Invalid global motion status updates input: "
                         "duplicate vehicle IDs in momentary status updates")
    if len(vehicle_ids_with_confirmed) != len(container.confirmed.updates):
        raise ValueError("Invalid global motion status updates input: "
                         "duplicate vehicle IDs in confirmed status updates")

    # arrange into a list by vehicle id and return
    vehicle_id_to_info: Dict[str, MotionStatusInfoForVehicle] = {
        vehicle_id: MotionStatusInfoForVehicle(vehicle_id=vehicle_id,
                                               momentary=momentary,
                                               confirmed=vehicle_id_to_confirmed_status[vehicle_id])
        for vehicle_id, momentary in vehicle_id_to_momentary_status.items()
    }
    return list(vehicle_id_to_info.values())
...
class LiveStateUpdater:
    ...
    def _get_track_id_by_platform_id(self, platform_id: str) -> str:
        if not platform_id in self.zones.ids_platform_to_track:
            raise ValueError(f"Zone ID {platform_id} for a platform is not found in platform-to-track "
                             f"zone ID mappings (not a platform?)")
        track_id: str = self.zones.ids_platform_to_track[platform_id]
        return track_id
    ...
    def _update_vehicle_motion_status(
            self, *, vehicle_id: str, event_ts: float, momentary: MotionStatus, confirmed: MotionStatus
    ) -> None:
        self._check_vehicle_initialised(vehicle_id)
        vehicle_state: VehicleState = self._get_vehicle_state(vehicle_id)
        vehicle_state.update_motion_status(
            momentary=momentary, confirmed=confirmed, event_ts=event_ts
        )
    ...
    def _update_from_speed_events(self, events: List[SpeedUpdateEvent]) -> None:
        for e in events:  # type: SpeedUpdateEvent
            self._update_vehicle_speeds(vehicle_id=e.vehicle_id,
                                        ...
                                        event_ts=self._internal_state.cur_frame_ts)

    # --- (3) global motion status UPDATES ---

    def _update_from_motion_status_updates(self, events: MotionStatusUpdatesContainer) -> None:
        info_for_all_vehicles: List[MotionStatusInfoForVehicle] = (
            _motion_status_updates_container_arrange_by_vehicle(events)
        )
        for item in info_for_all_vehicles: # type: MotionStatusInfoForVehicle
            self._update_vehicle_motion_status(vehicle_id=item.vehicle_id,
                                               event_ts=self._internal_state.cur_frame_ts,
                                               momentary=item.momentary,
                                               confirmed=item.confirmed)

    # --- (4) global motion status CHANGES (global) ---
    # start/end for stationary only

    def _update_from_global_stationary_start_events(self, events: List[StationaryStartEvent]) -> None:
        for event in events: # type: StationaryStartEvent
            vehicle_state: VehicleState = self.vehicles.states[event.vehicle_id]
            vehicle_state.start_confirmed_stationary(self._internal_state.cur_frame_ts)

    def _update_from_global_stationary_end_events(self, events: List[StationaryEndEvent]) -> None:
        for event in events: # type: StationaryEndEvent
            vehicle_state: VehicleState = self.vehicles.states[event.vehicle_id]
            vehicle_state.end_confirmed_stationary(self._internal_state.cur_frame_ts)

    def _update_from_global_motion_change_events(self, events: GlobalMotionStatusPeriodBoundaryEventsContainer) -> None:
        # NOTE: Only STATIONARY start/end are tracked in this live state updater
        self._update_from_global_stationary_start_events(events.start.stationary)
        self._update_from_global_stationary_end_events(events.end.stationary)

    # --- (5) zone transit ---

    @staticmethod
    def _arrange_zone_transit_events_by_zone_id(
    ...
    def _update_from_zone_transit_events(self, events: ZoneTransitEventsContainer) -> None:
        # arrange by zone id
        # [ ( zone_id, (vehicle_ids_to_add, vehicle_ids_to_remove) ), ... ]
        by_zone_id: List[ZoneStateUpdate] = self._arrange_zone_transit_events_by_zone_id(events)
    ...
    # --- (6) in-zone motion status changes ---
    # only for stationary periods (start/end)

    @staticmethod
    def _extract_inzone_stationary_and_arrange_by_zone_id(
            events: InZoneMotionStatusPeriodBoundaryEventsContainer
    ) -> List[ZoneStateUpdate]:
        # zone_id -> (vehicle_ids_to_add, vehicle_ids_to_remove)
        mappings: DefaultDict[str, VehicleIdSetsForZoneStateUpdate] = defaultdict(VehicleIdSetsForZoneStateUpdate)
        for start_event in events.start.stationary:  # type: InZoneStationaryStartEvent
            mappings[start_event.zone_id].to_add.add(start_event.vehicle_id)
        for end_event in events.end.stationary:  # type: InZoneStationaryEndEvent
    ...
    def _update_from_inzone_motion_change_events(
            self, events: InZoneMotionStatusPeriodBoundaryEventsContainer
    ) -> None:
        # NOTE: Only STATIONARY start/end are tracked in this live state updater
        by_zone_id: List[ZoneStateUpdate] = self._extract_inzone_stationary_and_arrange_by_zone_id(events)
        for container in by_zone_id:  # type: ZoneStateUpdate
    ...
    # --- (1) agnostic containers ---
    # agnostic wrt vehicle/zone types, i. e. not arranged by vehicle type, zone type
    ...
    def _export_agnostic_vehicles(self) -> AgnosticVehiclesContainer:
        ...
        for vehicle_id, vehicle_state in self.vehicles.states.items(): # type: str, VehicleState
            ...
            for intermediary_zone in intermediary_vehicle.zones.all_zones: # type: ExportFromVehicleZonesInfoItem
                ...
                zone_numerical_id: int = self.zones.id_to_num_id[zone_id]
                description: str = self.zones.id_to_description[zone_id]
            ...
            dest_vehicle: AgnosticVehicle = AgnosticVehicle(
                ...
                motion=intermediary_vehicle.motion,
    ...
    def _export_metadata(self) -> LiveStateMetadata:
        ...
        server_settings: SceneEventsConfig = cur_state.cur_server_settings
    ...
    # --- (2) non-agnostic containers for output ---
    # arranged by vehicle type, zone type
    # + containing type-specific info (currently just the track's ID and numerical ID for platforms)
    ...
    def _build_output_track_info_for_vehicle(
        ...
        return TrackInfoForVehicle(zone_id=agnostic_zone_info.zone_id,
                                   ...
                                   description=agnostic_zone_info.description,
    ...
    def _build_output_platform_info_for_vehicle(
        ...
        platform_id: str = agnostic_zone_info.zone_id
        track_id: str = self._get_track_id_by_platform_id(platform_id)
        return PlatformInfoForVehicle(zone_id=agnostic_zone_info.zone_id,
                                      zone_numerical_id=agnostic_zone_info.zone_numerical_id,
                                      description=agnostic_zone_info.description,
                                      track_zone_id=track_id,
                                      present_in_zone_since_ts=agnostic_zone_info.present_in_zone_since_ts,
                                      speed_in_zone_stats=agnostic_zone_info.speed_in_zone_stats)
    ...
    def _build_output_platform(self, agnostic_zone: AgnosticZone) -> Platform:
        ...
        platform_id: str = src_metadata.zone_id
        track_zone_id: str = self._get_track_id_by_platform_id(platform_id)
    ...
    def _build_output_tram(self, agnostic_vehicle: AgnosticVehicle) -> Tram:
        ...
        return Tram(vehicle_id=agnostic_vehicle.vehicle_id,
                    ...
                    motion=agnostic_vehicle.motion,
    ...
    # TODO: refactor to store the settings in the instance state
    def update_and_export_state(
            self, *, events: EventsContainer, settings: SceneEventsConfig
    ) -> LiveAnalyticsState:
        ...
        self._update_from_lifetime_start_events(events.pipeline_steps.vehicle_lifetime.start)
        ...
        self._update_from_speed_events(events.pipeline_steps.speeds.updates)

        # (3) motion status updates
        self._update_from_motion_status_updates(events.pipeline_steps.motion_status.motion_global.updates)

        # (4) global motion status changes
        # In this version of the updater, only the start/end of the current STATIONARY period
        # (as the one being of the greatest interest).
        #     - updates:
        #         * vehicle states: the previous stationary period start/end, the current stationary period start
        self._update_from_global_motion_change_events(
            events.pipeline_steps.motion_status.motion_global.period_boundary_events_for_confirmed_status
        )

        # (5) zone transit (entrances / exits)
        #     - updates:
        #         * zone transit substates: current and maximum vehicle counts, since when the zone has been occupied,
        #           the start and end of the previous occupancy period
        self._update_from_zone_transit_events(events.pipeline_steps.zone_transit)

        # (6) in-zone motion status changes
        # Likewise only the start/end of the current STATIONARY period.
        #     - updates:
        ...
        self._update_from_inzone_motion_change_events(events.pipeline_steps.motion_status.motion_in_zone)

        # (7) lifetime end
        #     MUST BE THE LAST
        #     - removes vehicle states for dead tracklets
        self._update_from_lifetime_end_events(events.pipeline_steps.vehicle_lifetime.end)
