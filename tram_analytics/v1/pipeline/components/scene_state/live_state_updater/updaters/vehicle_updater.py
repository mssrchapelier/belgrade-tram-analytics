...
from tram_analytics.v1.models.components.scene_state.live_state.vehicles import AgnosticZoneInfoForVehicle, \
    ...
    MotionInfoContainer
from tram_analytics.v1.pipeline.components.scene_state.live_state_updater.updaters.speed_updater import (
...
class VehicleSingleZoneInfo:
    ...
    for the duration of the vehicle's current transit through that zone
...
class VehicleMotionStatusTracker:
    ...
    def update_motion_status(self, *, event_ts: float, momentary: MotionStatus, confirmed: MotionStatus) -> None:
        if (
        ...
        last_update_ts: float | None = self._cur_motion_state.update_ts
        if last_update_ts is not None and event_ts <= last_update_ts:
            raise ValueError("Got an update for a vehicle's motion status with the timestamp "
                             "that is not strictly later than the last recorded update. "
    ...
    def end_confirmed_stationary(self, event_ts: float) -> None:
        ...
        if event_ts < self._cur_stationary_start_ts:
            raise ValueError("Got an update for a vehicle's global stationary period end with the timestamp "
                             "that is not strictly later than the recorded start "
                             "of the current stationary period start. "
                             "This is not yet supported")
        if event_ts > self._cur_stationary_start_ts:
            # update only if the duration is positive (is 0 when truncated)
...
class VehicleState:
    ...
    def update_speeds(self,
        ...
        self._lifetime_speeds_tracker.update(
            speed_raw=speed_raw, speed_smoothed=speed_smoothed, is_matched=is_matched, event_ts=event_ts
        )
        self._zone_presence_tracker.update_speeds_in_registered_zones(
            speed_raw=speed_raw, speed_smoothed=speed_smoothed, is_matched=is_matched, event_ts=event_ts
        )

    def update_motion_status(self, *, momentary: MotionStatus, confirmed: MotionStatus, event_ts: float) -> None:
        # NOTE: Does NOT start/end motion periods (this is handled by motion period start/end events)
        self._motion_status_tracker.update_motion_status(momentary=momentary,
                                                         confirmed=confirmed,
                                                         event_ts=event_ts)
