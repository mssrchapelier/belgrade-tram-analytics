...
class InZoneSingleVehicleBasedPeriodsUpdater(BaseInZoneVehiclePeriodsUpdater):
    ...
    def _check_before_removal(self, *, vehicle_ids: Set[str], event_ts: float) -> None:
        if self._prev_completed_period_times is not None and event_ts < self._prev_completed_period_times.end_ts:
            raise ValueError("Got period end that is earlier than the recorded end of the previous period")
        for vehicle_id in vehicle_ids:
    ...
    def _remove_vehicles(self, *, vehicle_ids: Set[str], event_ts: float) -> None:
        ...
        # filter start: leave only those vehicles
        # for which their period duration is positive (can be 0 if truncated)
        start_timestamps_filtered: List[float] = list(
            filter(lambda start_ts: start_ts < event_ts,
                   start_timestamps_for_removed)
        )
        # if the list is empty after filtering:
        # 1) either the vehicle_ids passed is empty -- do nothing then;
        # 2) or that means that for all of vehicles passed,
        # the corresponding period was started and then ended at the same frame
        # (due to a truncation -- truncated observation period or a spurious detection);
        # do not update the previous completed period timestamp container in that case
        if len(start_timestamps_filtered) > 0:
            # sort in ascending order (remove this if returning with vehicle ids is implemented in the future)
            start_timestamps_filtered.sort()
            self._prev_completed_period_times = PreviousInZoneSingleVehicleBasedPeriod(
                start_timestamps=start_timestamps_filtered,
                end_ts=event_ts
            )
...
class VehicleIdUpdater:
    ...
    def _add_vehicle_ids(self, *, vehicle_ids: Iterable[str], event_ts: float) -> None:
        ...
        self._id_to_insertion_timestamp.update({v_id: event_ts
    ...
    def _check_removal_not_earlier_than_insertion(self, *, event_ts: float, to_remove: Iterable[str]) -> None:
        for v_id in to_remove: # type: str
            insertion_ts: float = self._id_to_insertion_timestamp[v_id]
            if event_ts < insertion_ts:
                raise ValueError(f"Cannot remove vehicle_ids: "
                                 f"Removal timestamp is earlier than insertion timestamp for vehicle {v_id}. "
                                 f"Non-sequential updates are not yet supported")

    def _check_input_for_update(self, *, event_ts: float, to_add: Iterable[str], to_remove: Iterable[str]) -> None:
        ...
        self._check_removal_not_earlier_than_insertion(event_ts=event_ts, to_remove=to_remove)

    def update(self, *, event_ts: float, to_add: Iterable[str], to_remove: Iterable[str]) -> None:
        self._check_input_for_update(event_ts=event_ts, to_add=to_add, to_remove=to_remove)

        self._add_vehicle_ids(vehicle_ids=to_add, event_ts=event_ts)
...
class InZoneExistenceBasedPeriodsUpdater(BaseInZoneVehiclePeriodsUpdater):
    ...
    @override
    def update_with_vehicles(self, *, event_ts: float, to_add: Set[str], to_remove: Set[str]) -> None:
        self._vehicle_id_updater.update(event_ts=event_ts, to_add=to_add, to_remove=to_remove)
        ...
        if self._current_period is None and self._cur_vehicles > 0:
            ...
            if event_ts < self._current_period.start_ts:
                raise ValueError("Received a timestamp that is earlier than the stored current period's start. "
            ...
            if self._cur_vehicles > 0:
                ...
                completed_period_duration: float = event_ts - self._current_period.start_ts
                if completed_period_duration > 0:
                    # update only if the duration is positive (can be 0 if truncated)
...
class ZoneState:
    ...
    - `_single_vehicle_based_state`: vehicle passes (transits)
